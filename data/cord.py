...
class Cord(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        downloaded_file = '/root/share/datasets/cord/'
        # move files from the second URL together with files from the first one.
        # dest = Path(downloaded_file[0])/"CORD"
        dest = os.path.join('/root/share/datasets/cord/', 'CORD')
        ...
        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN, gen_kwargs={"filepath": os.path.join(dest, "train")}
            ),
            datasets.SplitGenerator(
                name=datasets.Split.VALIDATION, gen_kwargs={"filepath": os.path.join(dest, "dev")}
            ),
            datasets.SplitGenerator(
                name=datasets.Split.TEST, gen_kwargs={"filepath": os.path.join(dest, "test")}
            ),
        ]

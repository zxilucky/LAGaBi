...
class DataCollatorForKeyValueExtraction(DataCollatorMixin):
    ...
    sequence_cls: Optional[bool] = False
    ...
    def __call__(self, features):
        ...
        batch["input_ids"] = [x[:self.max_length] for x in batch["input_ids"]]
        ...
        has_segment_input = "segment_ids" in features[0]
        padding_idx=self.tokenizer.pad_token_id
        sequence_length = torch.tensor(batch["input_ids"]).shape[1] 
        padding_side = self.tokenizer.padding_side

        if padding_side == "right":
            if not self.sequence_cls:
            ...
            if has_segment_input:
                batch["segment_ids"] = [segment_ids + [padding_idx] * (sequence_length - len(segment_ids))
                                          for segment_ids in batch["segment_ids"]]
        else:
            if not self.sequence_cls:
            ...
            if has_segment_input:
                batch["segment_ids"] = [[padding_idx] * (sequence_length - len(segment_ids))
                                          + segment_ids for segment_ids in batch["segment_ids"]]
        for keys, values in batch.items():
            batch[keys] = [x[:self.max_length] for x in values]
